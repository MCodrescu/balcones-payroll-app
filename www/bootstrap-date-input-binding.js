...
// shiny.js.
var bootstrapDateInputBinding = new Shiny.InputBinding();


// An input binding must implement these methods
$.extend(bootstrapDateInputBinding, {
  ...
  subscribe: function(el, callback) {
    $(el).on('keyup.bootstrapDateInputBinding input.bootstrapDateInputBinding', function(event) {
      callback(true);
      // When called with true, it will use the rate policy,
      // which in this case is to debounce at 500ms.
    });
    $(el).on('change.bootstrapDateInputBinding', function(event) {
      callback(false);
  ...
  unsubscribe: function(el) {
    $(el).off('.bootstrapDateInputBinding');
  },
...
Shiny.inputBindings.register(bootstrapDateInputBinding, 'shiny.bootstrapDateInput');
